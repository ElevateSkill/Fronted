...
export default function FAQ({ faqs = [] }) {
  ...
  return (
    <div id="faq" className="relative w-full bg-black md:py-24 transition-colors duration-500 overflow-hidden">
      ...
      <div className="mx-auto relative z-10">
        <div className="text-center mb-16">
          <h2 className="text-4xl sm:text-5xl font-black tracking-tighter text-white mb-2">
          ...
          {displayFaqs.map((item, index) => (
            <motion.div
              ...
              className={`group overflow-hidden border transition-all duration-300 ${
                activeIndex === index 
                ? 'bg-white/5 border-[#fea305] shadow-xl shadow-[#15c8fb]/10' 
              ...
              >
                <div className="flex items-center gap-4">
                  <span className={`text-xs font-black px-3 py-1 rounded-lg transition-colors ${
                    activeIndex === index ? 'bg-[#fea305] text-white shadow-lg' : 'bg-white/10 text-gray-400'
                  }`}>
                    0{index + 1}
                  </span>
                ...
                <div className={`p-1.5 rounded-lg transition-all duration-300 ${
                  activeIndex === index ? 'rotate-180 bg-[#fea305] text-white shadow-lg' : 'bg-white/10 text-gray-400'
                }`}>
        ...
        <motion.div 
          ...
          whileInView={{ opacity: 1, y: 0 }}
          className="mt-16 p-8 md:p-12 bg-white/5 border border-white/10 text-center relative overflow-hidden group shadow-2xl shadow-white/5"
        >
          ...
          <div className="relative z-10">
            ...
            <div className="flex flex-wrap justify-center gap-4">
              ...
              <motion.button 
                whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
                className="flex items-center gap-3 px-8 py-4 bg-white/10 text-white font-black text-xs tracking-widest rounded-2xl uppercase border border-white/10 hover:border-white/20 transition-all shadow-xl"
              >
                Join Community
              </motion.button>
            </div>
