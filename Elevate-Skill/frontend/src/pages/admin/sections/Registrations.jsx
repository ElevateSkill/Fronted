...
export default function Registrations({ registrations, onApprove, onReject, onView }) {
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h2 className="text-2xl font-black text-brand-text tracking-tight">Registrations</h2>
          <p className="text-sm text-brand-muted font-medium mt-1">Manage user registrations</p>
        </div>
      ...
      {filtered.length === 0 ? (
        <EmptyState icon={UserPlus} title="No registrations found" description="No registrations match your search criteria." />
      ) : (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-brand-border bg-black/40">
                {['Full Name', 'Email', 'Phone', 'Course', 'Date', 'Status', 'Actions'].map(h => (
            ...
            <tbody>
              {filtered.map((r, i) => (
                ...
                >
                  <td className="p-4"><span className="font-semibold text-brand-text text-sm">{r.name}</span></td>
                  <td className="p-4 text-brand-muted text-xs">{r.email}</td>
                  <td className="p-4 text-brand-muted text-xs">{r.phone}</td>
                  <td className="p-4 text-brand-muted text-xs">{r.course}</td>
                  <td className="p-4 text-brand-muted text-xs">{r.date || '—'}</td>
                  <td className="p-4"><StatusBadge status={r.status} /></td>
                  <td className="p-4">
                    <div className="flex items-center justify-end gap-2">
                      <button onClick={() => onApprove(r.id)} disabled={r.status === 'Approved'} className="p-2 rounded-lg bg-brand-violet/20 text-brand-violet hover:bg-brand-violet/30 disabled:opacity-30 disabled:cursor-not-allowed transition-all cursor-pointer" title="Approve"><CheckCircle size={16} /></button>
                      <button onClick={() => onReject(r.id)} disabled={r.status === 'Rejected'} className="p-2 rounded-lg bg-brand-red/20 text-brand-red hover:bg-brand-red/30 disabled:opacity-30 disabled:cursor-not-allowed transition-all cursor-pointer" title="Reject"><XCircle size={16} /></button>
