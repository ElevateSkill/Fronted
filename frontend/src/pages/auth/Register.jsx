    ...
    >
      ...
      <div className="relative group">
        {Icon && (
          <Icon
            size={18}
            className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-[#5A2DA8] transition-all duration-300"
          />
        )}
        <input
          name={name}
          ...
          className={`w-full ${Icon ? 'pl-11' : 'pl-4'} ${isShowable ? 'pr-11' : 'pr-4'} py-3.5 bg-white .06] border border-gray-200 .12] rounded-2xl focus:outline-none focus:border-[#5A2DA8] focus:ring-2 focus:ring-[#5A2DA8]/20 transition-all duration-300 text-base placeholder:text-gray-400 :text-white/30 `}
        />
        {isShowable && (
          <button
            type="button"
            onClick={() => setLocalShow(!localShow)}
            className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400  hover:text-gray-600 :text-white/60"
          >
...
export default function Register() {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      const regRole = (regResult?.user?.role || 'student').toLowerCase();
      localStorage.setItem('elevate_user_role', regRole);
      setTimeout(() => {
  ...
  return (
    <div className="min-h-screen flex bg-gradient-to-br from-[#f0f4f8] via-[#e6ebf3] to-[#d1d9e8]    overflow-hidden font-sans relative">
      {/* Background Ornaments */}
      <div className="absolute inset-0 pointer-events-none">
        <motion.div animate={{ rotate: 360 }} transition={{ duration: 140, repeat: Infinity, ease: "linear" }}
          className="absolute -top-52 -left-52 w-[700px] h-[700px] border border-[#5A2DA8]/10 rounded-full" />
        <motion.div animate={{ rotate: -360 }} transition={{ duration: 160, repeat: Infinity, ease: "linear" }}
          className="absolute -bottom-64 -right-64 w-[800px] h-[800px] border border-[#3A3992]/10 rounded-full" />
      </div>
      ...
      <div className="hidden lg:flex lg:w-[46%] relative overflow-hidden bg-black">
        ...
        <div className="relative z-10 p-8 xl:p-12 flex flex-col justify-between h-full w-full">
          <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="flex items-center gap-5">
            ...
            <div>
              <span className="text-6xl xl:text-7xl font-black tracking-[-3px] text-white">Elevate<span className="text-[#5A2DA8]">Skill</span></span>
          ...
          <div className="space-y-6">
            <div className="inline-flex items-center gap-2 px-5 py-2 bg-white/10 backdrop-blur-3xl border border-white/30 rounded-3xl">
              <Sparkles size={20} className="text-[#3A3992]" />
            ...
            <h1 className="text-5xl xl:text-6xl font-black tracking-tighter text-white leading-none">
              Start Your<br />
              <span className="bg-gradient-to-r from-[#EE8433] via-white to-[#5A2DA8] bg-clip-text text-transparent">Engineering Journey</span>
            </h1>
          ...
          <div className="flex items-center gap-4 text-sm">
            <ShieldCheck size={28} className="text-[#5A2DA8]" />
      ...
      <div className="flex-1 flex items-center justify-center p-4 md:p-8 relative z-10">
        ...
        <Link
          to="/"
          className="absolute top-6 left-6 z-20 flex items-center gap-2 px-4 py-2 rounded-xl bg-white/80  backdrop-blur border border-gray-200  text-gray-700  hover:bg-white :bg-white/20 transition-all text-sm font-bold shadow-lg"
        >
          ...
          <div className="bg-white/95 backdrop-blur-3xl border border-[#EE8433]/20 rounded-3xl p-8 md:p-10 shadow-2xl shadow-[#EE8433]/10 relative overflow-hidden">
            ...
            <div className="flex gap-2 mb-8">
              {steps.map((s) => (
                <motion.div
                  ...
                  className={`flex-1 h-1.5 rounded-full cursor-pointer transition-all ${currentStep >= s.id ? 'bg-gradient-to-r from-[#EE8433] to-[#5A2DA8]' : 'bg-gray-200 '}`}
                />
              ))}
            </div>
              ...
              {msg && (
                <motion.div
                  ...
                  className={`mb-6 p-4 rounded-2xl text-sm flex gap-3 border ${msg.type === 'success'
                    ...
                    : 'bg-[#FEF0EE]  border-red-300  text-red-700 '
                    }`}
                >
            ...
            <form onSubmit={onSubmit} className="space-y-6">
              ...
              >
                <div className="flex items-center gap-2 mb-4">
                  <BookOpen size={18} className="text-[#3A3992]" />
                  <p className="text-sm font-semibold uppercase tracking-widest text-gray-500 ">Course & Payment</p>
                </div>
                ...
                <div className="space-y-5">
                  <div className="relative">
                    ...
                    <select
                      ...
                      className="w-full pl-11 pr-4 py-4 bg-white .06] border border-gray-200 .12] rounded-2xl focus:border-[#3A3992] focus:ring-2 focus:ring-[#3A3992]/20 text-base appearance-none "
                    >
                  ...
                  <label className={`group flex items-center gap-4 p-5 border-2 border-dashed rounded-2xl cursor-pointer transition-all duration-300 hover:border-[#3A3992] bg-white .04] ${proofFile ? 'border-green-400 bg-green-50 ' : 'border-gray-200 .12]'}`}>
                    <Upload size={28} className="text-gray-400  group-hover:text-[#3A3992] transition-colors" />
              ...
              <motion.button
                ...
                type="submit"
                disabled={loading}
                className="w-full py-4 mt-4 bg-gradient-to-r from-[#3A3992] via-[#3A3992] to-[#3A3992] text-white font-semibold text-lg rounded-2xl shadow-xl shadow-[#3A3992]/30 hover:shadow-[#3A3992]/50 flex items-center justify-center gap-3 disabled:opacity-70 transition-all"
              >
                {loading ? (
                  <>
            ...
            <div className="mt-8 text-center">
              <p className="text-sm text-gray-500 ">
                ...
                <Link to="/login" className="font-bold text-[#5A2DA8] hover:text-[#EE8433]">Sign in</Link>
              </p>
            </div>
          </div>
        </motion.div>
